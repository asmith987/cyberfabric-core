...
Proxy operations execute API calls to external services. Requests are routed from API Handler → Control Plane → Data Plane (config resolution) → Control Plane (execution).
...
- Routes `/api/oagw/v1/proxy/*` requests to Control Plane
...
#### 2. Control Plane: Upstream Resolution
...
#### 3. Control Plane: Route Resolution
...
#### 4. Control Plane: Plugin Execution
...
#### 7. Control Plane: Response Transform
