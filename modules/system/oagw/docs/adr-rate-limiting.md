...
**Component Architecture Note**: Rate limiting executes in **Control Plane (CP)**. CP owns rate limiters per-instance for MVP. Configuration is resolved from Data Plane (DP) caches
...
**MVP Implementation**: Per-instance rate limiting in Control Plane (no distributed coordination). Each CP instance maintains local token buckets. Acceptable for MVP since traffic
