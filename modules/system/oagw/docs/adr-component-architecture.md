...
- **Routes**:
    - `/api/oagw/v1/proxy/*` → Control Plane
    - `/api/oagw/v1/upstreams/*`, `/routes/*`, `/plugins/*` → Data Plane

**2. Control Plane (`oagw-cp`)**

- **Responsibility**: Orchestrate proxy requests to external services
- **Functions**:
    - Call Data Plane for config resolution (upstream, route)
...
- **Dependencies**: Data Plane (config resolution), cred_store (secret retrieval)
...
- [ADR: Data Plane Caching](./adr-data-plane-caching.md) - Multi-layer cache strategy
