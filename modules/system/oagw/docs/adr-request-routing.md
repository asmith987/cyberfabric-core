...
With three components (API Handler, Control Plane, Data Plane), we need to define how requests are routed between them. Different request types have different requirements:
...
|----------------------------|---------------|----------------|
| `/api/oagw/v1/upstreams/*` | Data Plane    | Upstream CRUD  |
| `/api/oagw/v1/routes/*`    | Data Plane    | Route CRUD     |
| `/api/oagw/v1/plugins/*`   | Data Plane    | Plugin CRUD    |
| `/api/oagw/v1/proxy/*`     | Control Plane | Proxy requests |
...
→ Data Plane (validate, write DB, invalidate cache)
→ Response
```
...
Client
→ API Handler (auth, rate limit)
→ Control Plane (orchestrate)
  → Data Plane (resolve upstream config)
  → Data Plane (resolve route config)
...
- [ADR: Data Plane Caching](./adr-data-plane-caching.md)
